...
def get_imgs_roi(img_file):
    ...
    for i, image in enumerate(images):
        ...
        try:
            ...
            print(f"Warning: Skipping {image} due to error in positioning_middle_point: {e}")
            continue
...
def FLANN_based_Matcher(img1_path, img2_path):
    ...
    if des1 is None or des2 is None or len(des1) < 2 or len(des2) < 2:
        return 0.0  # or np.nan or another value to indicate failure
...
# Step 1: ROI from grayscale bmp images (generate with convert_to_bmp.py)
#get_imgs_roi('./input/genuineORG_bmp_gray')

def match_roi(roi1, roi2, method):
    if method == 'FLANN':
        score = FLANN_based_Matcher(roi1, roi2)
    elif method == 'BIN':
        score = bin_match(roi1, roi2)
    else:
        raise ValueError("Method must be either 'FLANN' or 'BIN'")
    return score
